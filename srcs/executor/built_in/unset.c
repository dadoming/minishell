...
/*   Updated: 2023/03/17 21:39:57 by dadoming         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**unset(char **arg, char **env)
{
	...
	while (env[i])
	{
		if (string()->_compare_n(env[i], arg[1], \
			string()->_length_until_c(env[i], '=')) == 0)
			num_deletes++;
		i++;
	}
	new_env = (char **)malloc((sizeof(char *) \
		* (_array_length(env) - num_deletes + 1)));
...
static int	word_found(char *word, char **array)
{
	...
	while (array[i])
	{
		if (string()->_compare_n(array[i], word, \
			string()->_length_until_c(array[i], '=')) == 0)
			return (1);
		i++;
	}
	return (0);
