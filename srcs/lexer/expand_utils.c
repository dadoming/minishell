...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/03/16 00:15:59 by dadoming          #+#    #+#             */
/*   Updated: 2023/03/16 03:15:19 by dadoming         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	char **content, int *active_quote)
{
	...
	if (mini->core->env_p[norm.i][norm.env_len] == '=')
		*content = replace(content, norm.env_variable, \
			norm.env_value + 1);
	...
	int *active_quote, char *variable)
{
	...
	{
		norm.env_len = string()->_length_until_c \
			(mini->core->env_p[norm.i], '=');
		norm.env_variable = copy_until(mini->core->env_p[norm.i], norm.env_len);
		if (string()->_compare_n(norm.env_variable, variable, \
			norm.var_len) == 0 && norm.var_len == norm.env_len + 1)
		{
			free(variable);
	...
	int *active_quote, int curr_pos)
{
	...
	*content = replace(content, variable, env_value);
	free(env_value);
...
{
	*content = replace(content, variable, "");
	return (next_dollar_position(*content, active_quote, curr_pos));
}
